...
`packages/arch` 是 Coze Studio 前端架构的核心基础设施层，提供了构建整个应用所需的底层能力。该层包含 **37 个包**（统计口径：`frontend/packages/arch/` 目录下 `package.json` 文件数量），涵盖 API 通信、状态管理、国际化、日志监控、工具函数等关键领域。
...
├─────────────────────────────────────────────────────────────┤
│              Studio核心业务层 (packages/studio)                │
├─────────────────────────────────────────────────────────────┤
│              工作流引擎层 (packages/workflow)                  │
├─────────────────────────────────────────────────────────────┤
│                通用组件层 (packages/common)                    │
├─────────────────────────────────────────────────────────────┤
│  ★ 架构基础设施层 (packages/arch) ★                          │
...
- **高度复用**：被 monorepo 中的上层包依赖（参考 `rush.json` 中的 `level-1` 标签）
